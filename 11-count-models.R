...
poiss_fit <- glm(satellites ~ mass,
                data = crabs,
                family = poisson)

nb_fit <- glm(satellites ~ mass,
              data = crabs,
              family = MASS::negative.binomial(theta = 10)
              )
...
lmod <- glm(present ~ mass, data = crabs, family = binomial)

# Step 2 (a count model)
crabs_present <- crabs[crabs$present == 1,]

cmod <- glm(satellites ~ mass, data = crabs_present,
            family = MASS::negative.binomial(theta = 10))

# Predictions ----
# Step One (presence-absence)
